...
class ProductTest extends TestCase
{
    public function testAddProduct()
    {
        ...
        for ($i=0; $i < count($products); $i++) { 
            if (file_exists(public_path().'/upload/'.$products[$i]['image']))
            {
                $path = public_path().'/upload/'.$products[$i]['image'];
                break;
            }
        }
    ...
    public function testDeleteProduct()
    {
        ...
        if (count($product) > 10) {
            if (fmod(count($product), 10) == 0) {
                $link = 'admin/products?page='.floor(count($product)/10);
            } else {
                $link = 'admin/products?page='.(floor(count($product)/10) + 1);
            }
        } else {
            $link = 'admin/products';
        }

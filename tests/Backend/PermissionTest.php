...
class PermissionTest extends TestCase
{
    ...
    public function testShowErrorWhenAddPermissionExists()
    {
    	...
    	if ($role->see == $has_role && $role->insert == $has_role && $role->edit == $has_role && $role->destroy == $has_role) {
			$this->actingAs($admin, 'admin')
	    	 	 ->visit('admin/permissions')
	    		 ->click('create')
	    		 ->seePageIs('admin/permissions/create')
    ...
    public function testCancelDeletePermission()
    {
    	...
    	$this->actingAs($admin, 'admin')
    		 ...
    		 ->see('Are you sure delete this permission?')
			 ->press('Cancel')
    		 ->seeInDatabase('permissions', ['module' => $permission->module, 'see' => $permission->see, 'insert' => $permission->insert, 'edit' => $permission->edit, 'destroy' => $permission->destroy]);
    }
    ...
    public function testAddPermission()
    {
    	...
    	$this->actingAs($admin, 'admin')
    		 ...
    		 ->check('update')
    		 ->press('Create')
    		 ->seeInDatabase('permissions', ['module' => 'Brands', 'see' => 0, 'insert' => 1, 'edit' => 1, 'destroy' => 0])
    		 ->seePageIs('admin/permissions')
